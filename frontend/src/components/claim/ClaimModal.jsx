...
const ClaimModal = ({ foundItemId, foundItemCategory, verificationQuestions, isRetry = false, originalClaimId = null, onSuccess, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isRetry) {
      claimMutation.mutate({ originalClaimId, ownerAnswers });
    } else {
      claimMutation.mutate({ foundItemId, itemType: 'found', ownerAnswers });
    }
  };
            ...
            {step === 'questions' && (
              <motion.div key="questions" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-bold text-slate-800">Prove This Is Your Item</h2>
                  <p className="text-sm text-slate-500 mt-2">
                    Answer these questions that the finder answered when they reported this item.
                  </p>
                </div>
                ...
                {loadingQuestions ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 text-sm text-amber-800">
                      <strong>Important:</strong> Answer from memory — do not guess. You get 2 attempts total.
                    </div>

                    {questions.map((q, idx) => (
                      <div key={idx} className="space-y-2">
                        <label className="block text-sm font-semibold text-slate-700">
                          <span className="inline-block bg-sky-100 text-sky-700 px-2 py-0.5 rounded text-xs mr-2">Q{idx + 1}</span>
                          {q.question}
                        </label>
                        {q.answerType === 'descriptive' ? (
                          <textarea
                            ...
                            className="w-full bg-slate-50 border border-slate-300 rounded-lg p-3 text-slate-900 focus:ring-2 focus:ring-sky-500 outline-none transition"
                          />
                        ) : (
                          <input
                            type="text"
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                      ...
                      <button type="submit" className="px-6 py-2 bg-sky-600 hover:bg-sky-700 text-white font-medium rounded-xl transition shadow-lg shadow-sky-200">
                        Submit Claim
                      </button>
                    </div>
                  </form>
              ...
              <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                {result.approved ? (
                  <div className="text-center">
                    ...
                    <h2 className="text-2xl font-bold text-slate-800 mb-2">Claim Approved! 🎉</h2>
                    <p className="text-slate-600 mb-8">
                      Your answers matched perfectly. You can now chat with the finder to arrange handover.
                    </p>
                    <button 
                      onClick={() => onSuccess(result.chatId)}
                  ...
                  <div className="text-center">
                    ...
                    <h2 className="text-xl font-bold text-slate-800 mb-2">Claim Rejected</h2>
                    <p className="text-slate-600 mb-8">
                      Your answers did not match the finder's answers. If you believe this is your item, please contact support.
                    </p>
